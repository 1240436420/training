...
{
loging=true;
doRawXmailRequest('login/exchangeticket',{enc:1,func:1,sid:getSid(),cookie_sid:getCookie('sid')},'',function(e,d){
loging=false;
if(e&&d)
{
var c=JSON.parse(d);
if(c&&c.head&&c.head.ret===0&&c.body)
{
setCookie('xm_sid',c.body.xm_sid,null,'/','mail.qq.com',true,false);
setCookie('xm_skey',c.body.xm_skey,null,'/','mail.qq.com',true,false);
setCookie('xm_uin',c.body.xm_uin,null,'/','mail.qq.com',true,false);
}
else{
b='xmailAutoLogin failed! ret: '+(c&&c.head&&c.head.ret);
}
}
else{
b='xmailAutoLogin failed! network error!';
}
E(loginCallbacks,function(f){
if(f&&typeof f==='function')
{
f(b,{xmailSid:getCookie('xm_sid')});
}
});
loginCallbacks=[];
});
...
loadJsFileToTop(["$js_path$com/kits/qmeditor/qqmail/release/editor5055d5.js"]);
...
function checkSid(b,c)
{
var a={value:0};
var d=setInterval(function(){
if(a.value===c)
{
debug('checksid count up to limit: '+a.value+', abort!');
clearInterval(d);
return;
}
a.value+=1;
QMAjax.send('/cgi-bin/checksid?r='+Math.random(),{onload:function(f,e){
debug('checksid onload, success: '+f+', response: '+e+', date: '+new Date().toLocaleTimeString());
}});
},b);
}
